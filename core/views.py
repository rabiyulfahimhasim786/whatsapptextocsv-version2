...
def spamremovals_testing(request, string_param):
    try:
        ...
        spampatterns_to_avoid = patternstoavoiding()  
        for spamdatas in spampatterns_to_avoid:
            # print(spamdatas)
            # You need to define this function
            spamsearchdata = spamdatas
            results = Film.objects.raw(
                                "SELECT * FROM `core_film` WHERE filmurl LIKE %s AND dropdownlist=%s", [f'%{spamsearchdata}%', q])
                # "SELECT * FROM `core_film` WHERE filmurl LIKE %s AND dropdownlist=%s", [f'%{careersalessearchdata}%', 'New'])
            ...
            for user in results:
                ...
                try:
                    ...
                    with connection.cursor() as cursor:
                        cursor.execute("UPDATE core_film SET dropdownlist=%s  WHERE id = %s", ['Spam', user.id])
                    # print(f'Updated dropdownlist for id {user.id}')
                    message = f'Updated data for id {user.id}'
                except Film.DoesNotExist:
                    # print(f'Film object with id {user.id} does not exist')
                    message = f'Your data object with id {user.id} does not exist'
            # print(len(a))
        # return redirect('home')
        if q == 'New':
            return redirect('home')
...
def careersales_testing(request, string_param):
    ...
    for avoidcareerdatas in careersalestoconvertdata:
        # print(avoidcareerdatas)
        # You need to define this function
        avoidcareersearchdata = avoidcareerdatas
        results = Film.objects.raw("SELECT * FROM `core_film` WHERE filmurl LIKE %s AND dropdownlist=%s", [f'%{avoidcareersearchdata}%', q])
        # "SELECT * FROM `core_film` WHERE filmurl LIKE %s AND dropdownlist=%s", [f'%{careersalessearchdata}%', 'New'])
        # Update the age of all the users in the queryset
        # a =  []
        for user in results:
            ...
            try:
                # Retrieve the actual model instance
                with connection.cursor() as cursor:
                    cursor.execute("UPDATE core_film SET dropdownlist=%s  WHERE id = %s", ['Careersales', user.id])
                    # print(f'Updated dropdownlist for id {user.id}')
                    message = f'Updated data for id {user.id}'
            except Film.DoesNotExist:
                    # print(f'Film object with id {user.id} does not exist')
...
def benchsales_testing(request, string_param):
    ...
    # print(q)
        # Define your Q objects for filtering Film records
    benchsalestoconvertdata = benchsalestoconvert()  
    for avoidbenchdatas in benchsalestoconvertdata:
        # print(avoidbenchdatas)
        # You need to define this function
        avoidbenchsearchdata = avoidbenchdatas
        results = Film.objects.raw("SELECT * FROM `core_film` WHERE filmurl LIKE %s AND dropdownlist=%s", [f'%{avoidbenchsearchdata}%', q])
        # "SELECT * FROM `core_film` WHERE filmurl LIKE %s AND dropdownlist=%s", [f'%{careersalessearchdata}%', 'New'])
        # Update the age of all the users in the queryset
        # a =  []
        for user in results:
            ...
            try:
                # Retrieve the actual model instance
                with connection.cursor() as cursor:
                    cursor.execute("UPDATE core_film SET dropdownlist=%s  WHERE id = %s", ['Benchsales', user.id])
                    # print(f'Updated dropdownlist for id {user.id}')
                    message = f'Updated data for id {user.id}'
            except Film.DoesNotExist:
                    # print(f'Film object with id {user.id} does not exist')
