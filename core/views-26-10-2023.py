...
from .models import whatsapp, Film, Emailbenchsales, Emailleadoppurtunities, Customleads, Emailsample, CustomizeEmailLeads, CustomizeBenchsalesLeads, Customemailleads, Custombenchsales, Custombench, DuplicatedFilm, DuplicatedCustomemailleads, Aliasdata
        ...
        if form.is_valid():
            ...
            if option == 'whatsapp':
                ...
                with open(dot + 'media/data.csv', 'r', encoding="utf-8") as file:
                    ...
                    benchurl = 'https://career.desss.com/dynamic/careerdesssapi.php?action=get_table_values_based_namevalues&table=aliase_value_based_values&master_name=test%20bench'
                    ...
                    for row in reader:
                        ...
                        for pattern in patterns_to_avoid:
                            ...
                            # if re.search(rf'\b{re.escape(pattern)}\b', row[2], re.IGNORECASE):
                                should_skip = True
                                break
                        ...
                        if should_skip:
                            ...
                            if any(keyword.lower() in row[2].lower() for keyword in leadtuple):
                            # if any(re.search(rf'\b{re.escape(keyword)}\b', row[2], re.IGNORECASE) for keyword in leadtuple):
                            ...
                            elif any(keyword.lower() in row[2].lower() for keyword in benchtuple):
                            # elif any(re.search(rf'\b{re.escape(keyword)}\b', row[2], re.IGNORECASE) for keyword in benchtuple):
        ...
        if form.is_valid():
            ...
            if 'save_home' in request.POST:
                ...
                try:
                    ...
                    return render(request, 'navigation.html')
            elif 'careersales_save_next' in request.POST:
                try:
                    next_model = Film.objects.filter(id__gt=id).filter(dropdownlist='Careersales').order_by('id')[0]
                    

                    return redirect('whatsupcareersalesview', id=next_model.id)
                except IndexError:
                    return render(request, 'navigation.html')
            elif 'benchsales_save_next' in request.POST:
                try:
                    next_model = Film.objects.filter(id__gt=id).filter(dropdownlist='Benchsales').order_by('id')[0]
                    print(next_model)

                    return redirect('whatsupbenchsalesview', id=next_model.id)
                except IndexError:
                    return render(request, 'navigation.html')
...
def skillspattern(content):
    ...
    output_skills = unique_skills + [''] * (10 - len(unique_skills))
...
class Product_view(View):
    ...
    def get(self,  request):
        ...
        #     'details': details,
        #     'label_list': label_list,}
            details = Film.objects.filter(dropdownlist='New', checkstatus=1).order_by('-id')
            # details = Film.objects.filter(dropdownlist='New').order_by('-id')
    ...
    if request.method=="POST":
        ...
        if 'chiliadstaffingapi' in request.POST:
            ...
            # return redirect('customemailleads')
                ...
                leadskills6 = request.POST['leadskills6']
                leadexperience6 = request.POST['leadexperience6']
                leadoptional6= request.POST.get('leadoptional6', '')
                leadskills7 = request.POST['leadskills7']
                leadexperience7 = request.POST['leadexperience7']
                leadoptional7= request.POST.get('leadoptional7', '')
                leadskills8 = request.POST['leadskills8']
                leadexperience8 = request.POST['leadexperience8']
                leadoptional8= request.POST.get('leadoptional8', '')
                leadskills9 = request.POST['leadskills9']
                leadexperience9 = request.POST['leadexperience9']
                leadoptional9= request.POST.get('leadoptional9', '')
                leadskills10 = request.POST['leadskills10']
                leadexperience10 = request.POST['leadexperience10']
                leadoptional10= request.POST.get('leadoptional10', '')
                ...
                #return redirect('customemailleads')
                                # return redirect('search')
                #   return redirect('emailbenchsalesedit', id)
        elif 'skillsdeleteapi1' in request.POST:
                # new data fields
                leadskills1 = request.POST['leadskills1'] or ''
                #print(leademail)
                emailleadid = request.POST['emailleadid']
                phplead_id = request.POST['phplead_id']  or ''
                # print(leadskills1)
                # URL for the API endpoint
                url = 'https://career.desss.com/dynamic/careerdesssapi.php?action=name_based_delete_skills'
                # Sample data as a dictionary
                data = {
                    # 'action':'name_based_delete_skills',
                    'skill_name':leadskills1, 
                }
                # print(data)
                # Sending a POST request with the data
                response = requests.post(url, data=data)
                if response.status_code == 200:
                    ...
                    messages.success(request, 'Your skills has been deleted in Career.desss.com successfully!')  # Success message
                    return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
                   
                elif response.status_code == 401:
                    messages.success(request, 'Your skills has not been deleted in Career.desss.com successfully!')
                    return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
                    
                else:
                    messages.error(request, 'Your skills has not been deleted in Career.desss.com successfully!')  # Error message
                    return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
        elif 'skillsdeleteapi2' in request.POST:
                # new data fields 
                leadskills2 = request.POST['leadskills2'] or ''
                #print(leademail)
                emailleadid = request.POST['emailleadid']
                phplead_id = request.POST['phplead_id']  or ''
                # print(leadskills1)
                ...
                if response.status_code == 200:
                    ...
                    messages.error(request, 'Your skills has not been deleted in Career.desss.com successfully!')  # Error message
                    return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
        elif 'skillsdeleteapi3' in request.POST:
                # new data fields
                leadskills3 = request.POST['leadskills3'] or ''
                #print(leademail)
                emailleadid = request.POST['emailleadid']
                phplead_id = request.POST['phplead_id']  or ''
                # print(leadskills1)
                ...
                if response.status_code == 200:
                    ...
                    messages.error(request, 'Your skills has not been deleted in Career.desss.com successfully!')  # Error message
                    return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
        elif 'skillsdeleteapi4' in request.POST:
                # new data fields
                leadskills4 = request.POST['leadskills4'] or ''
                #print(leademail)
                emailleadid = request.POST['emailleadid']
                phplead_id = request.POST['phplead_id']  or ''
                # print(leadskills1)
                ...
                if response.status_code == 200:
                    ...
                    messages.error(request, 'Your skills has not been deleted in Career.desss.com successfully!')  # Error message
                    return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
        elif 'skillsdeleteapi5' in request.POST:
                # new data fields
                leadskills5 = request.POST['leadskills5'] or ''
                #print(leademail)
                emailleadid = request.POST['emailleadid']
                phplead_id = request.POST['phplead_id']  or ''
                # print(leadskills1)
                ...
                if response.status_code == 200:
                    ...
                    messages.error(request, 'Your skills has not been deleted in Career.desss.com successfully!')  # Error message
                    return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
        elif 'skillsdeleteapi6' in request.POST:
                # new data fields
                leadskills6 = request.POST['leadskills6'] or ''
                #print(leademail)
                emailleadid = request.POST['emailleadid']
                phplead_id = request.POST['phplead_id']  or ''
                # print(leadskills1)
                ...
                if response.status_code == 200:
                    ...
                    messages.error(request, 'Your skills has not been deleted in Career.desss.com successfully!')  # Error message
                    return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
        elif 'skillsdeleteapi7' in request.POST:
                # new data fields
                leadskills7 = request.POST['leadskills7'] or ''
                #print(leademail)
                emailleadid = request.POST['emailleadid']
                phplead_id = request.POST['phplead_id']  or ''
                # print(leadskills1)
                ...
                if response.status_code == 200:
                    ...
                    messages.error(request, 'Your skills has not been deleted in Career.desss.com successfully!')  # Error message
                    return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
        elif 'skillsdeleteapi8' in request.POST:
                # new data fields
                leadskills8 = request.POST['leadskills8'] or ''
                #print(leademail)
                emailleadid = request.POST['emailleadid']
                phplead_id = request.POST['phplead_id']  or ''
                # print(leadskills1)
                ...
                if response.status_code == 200:
                    ...
                    messages.error(request, 'Your skills has not been deleted in Career.desss.com successfully!')  # Error message
                    return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
        elif 'skillsdeleteapi9' in request.POST:
                # new data fields
                leadskills9 = request.POST['leadskills9'] or ''
                #print(leademail)
                emailleadid = request.POST['emailleadid']
                phplead_id = request.POST['phplead_id']  or ''
                # print(leadskills1)
                ...
                if response.status_code == 200:
                    ...
                    messages.error(request, 'Your skills has not been deleted in Career.desss.com successfully!')  # Error message
                    return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
        elif 'skillsdeleteapi10' in request.POST:
                # new data fields
                leadskills10 = request.POST['leadskills10'] or ''
                #print(leademail)
                emailleadid = request.POST['emailleadid']
                phplead_id = request.POST['phplead_id']  or ''
                # print(leadskills1)
...
def custombenchsalesedits(request,id):
    ...
    company_match = re.search(r'@([\w.-]+)', emailcontent)
    company_name = ''

    if company_match:
        # Extract the domain part
        email_domain = company_match.group(1)

        # Check if the domain contains "gmail" or "yahoo"
        if "gmail" not in email_domain and "yahoo" not in email_domain:
            # Extract the company name by removing the TLD
            company_name = email_domain.rsplit('.', 1)[0]
            print(company_name)
        else:
            # company_name = ''
            company_name = extract_companyinfo(email_content)
...
class whatsappcareersales_view(View):
    ...
    def get(self,  request):
        details = Film.objects.filter(dropdownlist='Careersales', checkstatus=1).order_by('id')
...
def extract_information(content, id):
    ...
    for pattern in it_role_patterns:
        ...
        #     role_details = ''
            ...
            # location = location_match.group(1).strip() if location_match else ""
            # print(role_details)
    position_pattern = re.compile(r'Job Title:\s(.*?)<\/p>')
            # location_pattern = re.compile(r'Location:\s(.*?)\n')
            #duration_pattern = re.compile(r'Duration:\s(.*?)\n')
    ...
    return first_name, email_id, phone_number, location, street,city, state, zip_code, legal_status, mobile_number, role_details, direct_number, company_name
...
def whatsappcareersalesedit(request, id):
    ...
    if int(php_code)==200:
        ...
        # print('error')
        first_name, email_id, phone_number, location, street,city, state, zip_code, legal_status, mobile_number,  role_details, direct_number, company_name= extract_information(content, id)
        extracted_data = {
         "id":id,
        "first_name": first_name,
        "email_id": email_id,
        ...
        if 'chiliadstaffingapi' in request.POST:
            ...
            return redirect('customemailleads')
                ...
                if response.status_code == 200:
                    ...
                    messages.error(request, 'Your lead has not been updated to Career.desss.com!')  # Error message
                    return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
        elif 'careersales_edit_save_next' in request.POST:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        try:
                            next_model = Film.objects.filter(id__gt=id).filter(dropdownlist='Careersales').order_by('id')[0]
                            return redirect('whatsupcareersalesedit', id=next_model.id)
                        except IndexError:
                            return render(request, 'navigation.html')
                        # return HttpResponse(leadlegalstatus)
                        ...
                        try:
                            ...
                            return render(request, 'navigation.html')
                    # next_model = Film.objects.filter(id__gt=id).filter(dropdownlist='Careersales').order_by('id')[0]
                    # return redirect('whatsappcareersalesedit', id=next_model.id)
                except IndexError:
                    return render(request, 'navigation.html')
...
class whatsappbenchsales_view(View):
    ...
    def get(self,  request):
        details = Film.objects.filter(dropdownlist='Benchsales', checkstatus=1).order_by('id')
...
def whatsappbenchsalesedit(request, id):
    ...
    try:
        ...
        if int(php_code)==200:
           ...
           context = {'d': benchdata, 'object':object_lead, 'whatsupleadsid':whatsupleadsid, 'indexhorizontal':index_horizontal, 'indexvertical':index_vertical, 'extracted_data':extracted_data, "email_id": email_id, "mobile_number": mobile_number, "phone_number": phone_number, 'bench_sale_lead_id':bench_sale_lead_id,"company_name":company_name,"is_status_check": 'This Email id or Mobile number is not exist in the Career site','table_message':table_message, "address":address_match,}
            #    data = keydata
        else:
            ...
            # index_horizontal = len(benchdata)
            # index_vertical = len(benchdata[0])
            context = {'d': benchdata, 'object':object_lead, 'whatsupleadsid':whatsupleadsid, 'indexhorizontal':index_horizontal, 'indexvertical':index_vertical, 'extracted_data':extracted_data, "email_id": email_id, "mobile_number": mobile_number, "phone_number": phone_number,"company_name":company_name,'bench_sale_lead_id':bench_sale_lead_id,"is_status_check": 'This Email id or Mobile number is not exist in the Career site','table_message':table_message, "address":address_match,}
        return render(request, 'whatsappbenchsalesedit.html', context)
...
def whatsappbenchsalescreate(request):
    if request.method=="POST":
        ...
        elif 'careerdesssapi_next' in request.POST:
            indexhorizontal = request.POST['indexhorizontal']
            indexvertical = request.POST['indexvertical']
            if int(indexhorizontal) == 1:
                ...
                if response.status_code == 200:
                    ...
                    try:
                        next_model = Film.objects.filter(id__gt=id).filter(dropdownlist='Benchsales').order_by('id')[0]
                        return redirect('whatsupbenchsalesedit', id=next_model.id)
                    except IndexError:
                        return render(request, 'navigation.html')
                    # return HttpResponse(leadlegalstatus)
...
def spam_removal(messagedata, yeardata):
    patterns_to_avoid = patternstoavoiding()  # You need to define this function

    should_skip = False
    # for pattern in patterns_to_avoid:
    #     if re.search(pattern, messagedata, re.IGNORECASE):should_skip = True
    #     break
    if any(keyword.lower() in messagedata.lower() for keyword in patterns_to_avoid):

        # existing_film = Film.objects.filter(year=yeardata, filmurl=messagedata).first()
        existing_film = Film.objects.get(year=yeardata, filmurl=messagedata,)
        if existing_film:
                # print(existing_film)
                # Update the existing film
                # existing_film.date = date_str

                # existing_film.title = time_str
                # existing_film.year = row[1]
                existing_film.dropdownlist = 'Spam'
                existing_film.save()
                return 'spam'
        else:
...
def spamremovals_testing(request, string_param):
    try:
        # q = 'New'
        q = string_param
        print(q)
        # Define your Q objects for filtering Film records
        q_objects = Q(filmurl__icontains=q)
        
        # You can add more conditions to q_objects if needed
        # For example, q_objects &= Q(another_field=some_value)
        
        # Fetch all Film records that match your criteria
        messagedata = Film.objects.filter(q_objects).filter(checkstatus=1)
        patterns_to_avoid = patternstoavoiding()  # You need to define this function
        if messagedata.exists():
            for film in messagedata:
                # Process each film record here
                # Extract the data you need, e.g., film.filmurl and film.year
                if any(keyword.lower() in film.filmurl.lower() for keyword in patterns_to_avoid):
                    result = spam_removal(film.filmurl, film.year)
                
                    if result == 'spam':
                        print(f"Film ID {film.id} is spam.")
                    else:
                        print(f"Film ID {film.id} is not spam.")
            # return redirect('home')
            if q == 'New':
                return redirect('home')
            elif q == 'Benchsales':
            ...
            # print("No matching Film records found in the database.")
            return redirect('home')
        return redirect('home')
    except Film.DoesNotExist:
        # print("An error occurred while processing Film records.")
        return redirect('home')
    return redirect('home')


def careersales_removal(messagedata, yeardata):
    # careersales_convert = careersalestoconvert()  # You need to define this function
    careersales_convert = careersalestoconvert()  
    should_skip = False
    # for pattern in patterns_to_avoid:
    #     if re.search(pattern, messagedata, re.IGNORECASE):should_skip = True
    #     break
    if any(keyword.lower() in messagedata.lower() for keyword in careersales_convert):

        existing_film = Film.objects.get(year=yeardata, filmurl=messagedata)
    
        # existing_film = Film.objects.get(filmurl=messagedata)
        if existing_film:
                # print(existing_film)
                # Update the existing film
                # existing_film.date = date_str
                # existing_film.title = time_str
                # existing_film.year = row[1]
                existing_film.dropdownlist = 'Careersales'
                existing_film.save()
                return 'Careersales'
        else:
                # Create a new film if it doesn't exist
...
def careersales_changer(q):
    try:
        # Define your Q objects for filtering Film records
        q_objects = Q(filmurl__icontains=q)
        # q_objects = Q(Q(checkstatus__icontains=1) | Q(filmurl__icontains=q))
        careersales_convert = careersalestoconvert()  # You need to define this function
        # You can add more conditions to q_objects if needed
        # For example, q_objects &= Q(another_field=some_value)
        ...
        # Fetch all Film records that match your criteria
        messagedata = Film.objects.filter(q_objects).filter(checkstatus=1)
        # messagedata = Film.objects.filter(q_objects)
   
        if messagedata.exists():
            for film in messagedata:
                # Process each film record here
                # Extract the data you need, e.g., film.filmurl and film.year
                if any(keyword.lower() in film.filmurl.lower() for keyword in careersales_convert):
                    result = careersales_removal(film.filmurl, film.year)
                    
                    if result == 'Careersales':
                        print(f"Film ID {film.id} is Careersales.")
                    else:
                        print(f"Film ID {film.id} is not Careersales.")
            return redirect('home')
...
def careersales_testing(request, string_param):
    # q = 'New'
        # Define your Q objects for filtering Film records
    q = string_param
    print(q)
    careersales_changer(q)
    # return redirect('home')
    if q == 'New':
        return redirect('home')
    elif q == 'Benchsales':
...
def benchsales_removal(messagedata, yeardata):
    benchsales_convert = benchsalestoconvert()  # You need to define this function

    should_skip = False
    # for pattern in patterns_to_avoid:
    #     if re.search(pattern, messagedata, re.IGNORECASE):should_skip = True
    #     break
    if any(keyword.lower() in messagedata.lower() for keyword in benchsales_convert):
        ...
        if existing_film:
                ...
                # existing_film.title = time_str
                # existing_film.year = row[1]
                existing_film.dropdownlist = 'Benchsales'
                existing_film.save()
                return 'Benchsales'
        else:
                # Create a new film if it doesn't exist
    ...
    try:
        ...
        if messagedata.exists():
            for film in messagedata:
                # Process each film record here
                # Extract the data you need, e.g., film.filmurl and film.year
                if any(keyword.lower() in film.filmurl.lower() for keyword in benchsales_convert):
                    result = benchsales_removal(film.filmurl, film.year)
                
                    if result == 'Benchsales':
                        print(f"Film ID {film.id} is Benchsales.")
                    else:
                        print(f"Film ID {film.id} is not Benchsales.")
            return redirect('home')
...
def benchsales_testing(request, string_param):
    # q = 'New'
    q = string_param
    print(q)
        # Define your Q objects for filtering Film records
    benchsales_changer(q)
    if q == 'New':
        return redirect('home')
    elif q == 'Benchsales':
        return redirect('whatsupbenchsales')
...
class Duplicatedwhatsappcareersales_view(View):
    def get(self, request, id):
        original_lead = Film.objects.get(id=id)
        # Duplicate the original lead and save it as a new entry
        duplicated_lead = Film.objects.create(
            date=original_lead.date,
            title=original_lead.title,
            year=original_lead.year,
            filmurl=original_lead.filmurl,
            companyname=original_lead.companyname,
            maincategory=original_lead.maincategory,
            subcategory=original_lead.subcategory,
            roles=original_lead.roles,
            contactdetails=original_lead.contactdetails,
            shortjobdescription=original_lead.shortjobdescription,
            responsibilities=original_lead.responsibilities,
            qualifications=original_lead.qualifications,
            comments=original_lead.comments,
            keywords=original_lead.keywords,
            benchreparse=original_lead.benchreparse,
            checkstatus=original_lead.checkstatus,
            dropdownlist=original_lead.dropdownlist,
        )

        # Redirect to the edit page for the duplicated lead with the new ID
        return redirect('whatsupcareersalesview', id=duplicated_lead.id)

class DuplicatedCustomEmailleadsView(View):
    def get(self, request, id):
        original_lead = Customemailleads.objects.get(id=id)
        
        # Find the count of existing duplicates with the same "Subject"
        duplicate_count = Customemailleads.objects.filter(
            Subject__icontains=f"Duplicate {original_lead.Subject}"
        ).count()

        # Create a new entry with a unique identifier
        duplicated_lead = Customemailleads.objects.create(
            todaysdate=original_lead.todaysdate,
            Subject=f"Duplicate {original_lead.Subject} ({duplicate_count + 1})",
            From=original_lead.From,
            Received_Time=original_lead.Received_Time,
            Email_Body=original_lead.Email_Body,
            Email_descriptions=original_lead.Email_descriptions,
            contactdetails=original_lead.contactdetails,
            shortjobdescription=original_lead.shortjobdescription,
            responsibilities=original_lead.responsibilities,
            qualifications=original_lead.qualifications,
        ...
        # Redirect to the edit page for the duplicated lead with the new ID
        return redirect('customemailleadsview', id=duplicated_lead.id)



def alias(request):
    if request.method == 'POST':
        masteralias = request.POST['masteralias']
        masteraliascolumn = request.POST['masteraliascolumn']
        masteraliasinput = request.POST['masteraliasinput']
        # print(masteralias, masteraliascolumn, masteraliasinput)
    # member = Aliasdata(masteralias=request.POST['masteralias'], masteraliascolumn=request.POST['masteraliascolumn'], masteraliasinput=request.POST['masteraliasinput'])
    # member.save()
        if masteraliasinput is not None:
            alias_url_params = f" https://career.desss.com/dynamic/careerdesssapi.php?action=insert_master_alias_name_based_name_values&master_alias={masteralias}&master_alias_column={masteraliascolumn}&master_alias_input={masteraliasinput}"
            aliasresponse = requests.get(alias_url_params)
