...
def save_candidate_info(email_content):
    ...
    # name_pattern = re.compile(r'(?:Candidate|Full) Name: (.+?)\n', re.IGNORECASE)
    name_pattern = re.compile(r'(?:Full Name of Candidate \(As per SSN/Passport\)|(?:Candidate|Full) Name): (.+?)<br />', re.IGNORECASE)
    ...
    # experience_pattern = re.compile(r'(?:Total (?:Experience|Relevant Experience)(?: in the USA)?: (\d+))', re.IGNORECASE)
    experience_pattern = re.compile(r'Total (?:Experience|Relevant Experience)(?: in the USA)?: (\d+\+?(?: Years)?)', re.IGNORECASE)
    experience_matches = experience_pattern.findall(email_content)
    experiences = experience_matches
    # print(experiences)


    # position_pattern = re.compile(r'Job Title:\s*(.+?)\s*\n', re.IGNORECASE)
    position_pattern = re.compile(r'Job Title:\s*([^<]+)', re.IGNORECASE)
    position_matches = position_pattern.findall(email_content)
    positions = position_matches
    # print(positions)

    # Extracting Location
    # location_pattern = re.compile(r'Current Location: (.+?)\n', re.IGNORECASE)
    # location_matches = location_pattern.findall(email_content)
    # locations = location_matches
    # print(locations)

    location_pattern = re.compile(r'Current Location: (.+?)<br />|Present Location \(City and State\): (.+?)<br />', re.IGNORECASE)
    location_matches = location_pattern.findall(email_content)

    locations = [location[0] if location[0] else location[1] for location in location_matches if any(location)]
    # print(locations)

    # Extracting Legal Status
    # legal_status_pattern = re.compile(r'Visa Status: (.+?)\n', re.IGNORECASE)
    # legal_status_matches = legal_status_pattern.findall(email_content)
    # legal_statuses = legal_status_matches
    # print(legal_statuses)

    legal_status_pattern = re.compile(r'Visa Status/Work Authorization: (.+?)<br />|Visa Status: (.+?)<br />', re.IGNORECASE)
    legal_status_matches = legal_status_pattern.findall(email_content)
    legal_statuses = [match[0] or match[1] for match in legal_status_matches]
    # print(legal_statuses)
    # Extracting Notice Period
    # notice_period_pattern = re.compile(r'Availability: (.+?)\n', re.IGNORECASE)
    notice_period_pattern = re.compile(r'(?:Notice Period/Availability to Join|Availability):\s*(.+?)\s*(?:<br />|\n)', re.IGNORECASE)
    ...
    # Modify the pattern to extract "Relocation (Yes/No)"
    relocation_pattern = re.compile(r'Relocation \(Yes/No\):\s*([^<]+)', re.IGNORECASE)
...
def custombenchsalesedits(request,id):
    ...
    email_designation = object_lead.bench_descriptions
...
def whatsappbenchsalesedit(request, id):
    ...
    try:
        ...
        if int(php_code)==200:
           ...
           context = {'d': benchdata, 'object':object_lead, 'whatsupleadsid':whatsupleadsid, 'indexhorizontal':index_horizontal, 'indexvertical':index_vertical, 'extracted_data':extracted_data, "email_id": email_id, "mobile_number": mobile_number, "phone_number": phone_number, 'bench_sale_lead_id':bench_sale_lead_id,"is_status_check": 'This Email id or Mobile number is not exist in the Career site','table_message':table_message}
            #    data = keydata
        else:
