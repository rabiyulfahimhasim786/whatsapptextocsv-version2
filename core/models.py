...
class DuplicatedFilm(models.Model):
    original_film = models.ForeignKey(Film, on_delete=models.CASCADE)
    # Add any additional fields you want to store for duplicated entries here

    def __str__(self):
        return f"Duplicated Film for {self.original_film.title}"
    

class DuplicatedCustomemailleads(models.Model):
    original_Customemailleads = models.ForeignKey(Customemailleads, on_delete=models.CASCADE)
    # Add any additional fields you want to store for duplicated entries here

    def __str__(self):
        return f"Duplicated Customemailleads for {self.original_Customemailleads.Subject}"
    
class Aliasdata(models.Model):
    masteralias =  models.TextField(blank=True)
    masteraliascolumn =  models.TextField(blank=True)
    masteraliasinput =  models.TextField(blank=True)

    def __str__(self):
        return self.masteralias
