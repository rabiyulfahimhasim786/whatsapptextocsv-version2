...
from .models import whatsapp, Film, Emaillead, Emailleadoppurtunities, Emailsample, CustomizeEmailLeads
from .forms import WhatsappForm, FilmForm, LocationChoiceField, LabelChoiceField, DateChoiceField, UploadFileForm, EmailleadForm, EmailleadoppurtunitiesForm, EmailsampleForm, CustomizeEmailLeadsForm
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if created:
                lead.save()
            # lead.save()
                        # emailleadspath =  dot+'media/emailleads.csv'
            # print(emailleadspath)
            # df = pd.read_csv(dot + 'media/emailleads.csv',  sep=",", encoding='utf8', low_memory=False)
            # for _, row in df.iterrows():
            #     if row[7] == '<Media omitted>' or row[7] == 'Waiting for this message':
            #             continue
            #     else:
            #         emailsample, created = Emailsample.objects.get_or_create(
            #             firstname=row[0],
            #             lastname=row[1],
            #             email=row[2],
            #             username=row[3],
            ...
            #         if created:
            #             emailsample.save()
            # with open(emailleadspath, 'r') as emailleadsfile:
            #     reader = csv.reader(emailleadsfile)
...
class Emailsamplelead_view(View):
    ...
    def get(self,  request):
        ...
        if 'q' in request.GET:
            ...
            # data = Film.objects.filter(filmurl__icontains=q)
            multiple_q = Q(Q(refusermob__icontains=q) | Q(refuserdetails__icontains=q))
            details = Emailsample.objects.filter(multiple_q).filter(Q(refdropdownlist='New'))
            # object=Film.objects.get(id=id)
        elif request.GET.get('locations'):
            selected_location = request.GET.get('locations')
            details = Emailsample.objects.filter(refcheckstatus=selected_location)
        elif request.GET.get('label'):
            labels = request.GET.get('label')
            details = Emailsample.objects.filter(refdropdownlist=labels)#.filter(dropdownlist='New')
        elif request.GET.get('datesdata'):
            selected_datedata = request.GET.get('datesdata')
            details = Emailsample.objects.filter(refdropdownlist='New',title=selected_datedata)
        else:
