...
def documentationuploads(request):
    folder=filedot+'helpdocumentation/' 
    try:
        if request.method == 'POST' and request.FILES['myfile']:
                myfile = request.FILES['myfile']
                fs = FileSystemStorage(location=folder) #defaults to   MEDIA_ROOT  
                  # Check if a file with the same name exists in the folder and delete it.
                existing_file_path = os.path.join(folder, myfile.name)
                if os.path.exists(existing_file_path):
                    print('completed')
                    os.remove(existing_file_path)
                filename = fs.save(myfile.name, myfile)
                file_url = fs.url(filename)
                errortext = 'File uploaded successfully'
                return render(request, 'contents/upload_document.html', {
                    'file_url': file_url, 'errortext': errortext,
                })
        else:
                return render(request, 'contents/upload_document.html')
    except:
        errortext = 'Please choose your file'
        return render(request, 'contents/upload_document.html', {'errortext': errortext,})